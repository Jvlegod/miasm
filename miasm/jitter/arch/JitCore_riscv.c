...
reg_dict gpreg_dict[] = {
    ...
    {.name = "X30", .offset = offsetof(struct vm_cpu, X30), .size = 64},
    {.name = "X31", .offset = offsetof(struct vm_cpu, X31), .size = 64},
...
PyObject* cpu_get_gpreg(JitCpu* self)
{
    ...
    get_reg(exception_flags);
    get_reg(interrupt_num);
...
void dump_gpregs(struct vm_cpu* vmcpu)
{
    printf("X0  %.16"PRIX64" X1  %.16"PRIX64" X2  %.16"PRIX64" X3  %.16"PRIX64"\n",
           vmcpu->X0, vmcpu->X1, vmcpu->X2, vmcpu->X3);
    printf("X4  %.16"PRIX64" X5  %.16"PRIX64" X6  %.16"PRIX64" X7  %.16"PRIX64"\n",
           vmcpu->X4, vmcpu->X5, vmcpu->X6, vmcpu->X7);
    printf("X8  %.16"PRIX64" X9  %.16"PRIX64" X10 %.16"PRIX64" X11 %.16"PRIX64"\n",
           vmcpu->X8, vmcpu->X9, vmcpu->X10, vmcpu->X11);
    printf("X12 %.16"PRIX64" X13 %.16"PRIX64" X14 %.16"PRIX64" X15 %.16"PRIX64"\n",
    ...
    printf("X20 %.16"PRIX64" X21 %.16"PRIX64" X22 %.16"PRIX64" X23 %.16"PRIX64"\n",
    ...
    printf("X28 %.16"PRIX64" X29 %.16"PRIX64" X30 %.16"PRIX64" X31 %.16"PRIX64"\n",
           vmcpu->X28, vmcpu->X29, vmcpu->X30, vmcpu->X31);

    printf("PC  %.16"PRIX64" exception_flags %08"PRIX32" interrupt_num %08"PRIX32"\n",
           vmcpu->PC,
           vmcpu->exception_flags,
           vmcpu->interrupt_num);
}
...
static PyGetSetDef JitCpu_getseters[] = {
    ...
    {"X30", (getter)JitCpu_get_X30, (setter)JitCpu_set_X30, "X30", NULL},
    {"X31", (getter)JitCpu_get_X31, (setter)JitCpu_set_X31, "X31", NULL},
...
static PyMethodDef JitCore_riscv_Methods[] = {
    {"get_gpreg_offset_all", (PyCFunction)get_gpreg_offset_all, METH_NOARGS},
    {NULL, NULL, 0, NULL}
};

MOD_INIT(JitCore_riscv)
{
    PyObject *module = NULL;

    MOD_DEF(module,
            "JitCore_riscv",
            "JitCore_riscv module",
            JitCore_riscv_Methods);
    ...
    if (PyModule_AddObject(module, "JitCpu",
                           (PyObject *)&JitCpuType) < 0)
        RET_MODULE;
